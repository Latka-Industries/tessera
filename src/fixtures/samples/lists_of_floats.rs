...
/// Document with `\lof` / `\lot` before captioned figures and tables.
///
/// Pair with `page_chrome` so page digits resolve on the list lines.
///
/// # Panics
///
...
pub fn encode_lists_of_floats() -> Vec<u8> {
    ...
    session
        .add_text_chunk(
            ...
            "Front matter lists expand from later captioned figures and tables \
             (Tessprek \\lof / \\lot). Export with page_chrome to see page digits.",
        )
        .expect("intro");
    ...
    add_flow_figure(
        ...
        "Second swatch",
        None,
        Some("Caption-only figure (no title) still appears in the list."),
    )
    .expect("figure 2");
    ...
    table_a.caption = Some("First table caption for the LOT.".into());
    ...
    table_b.caption = Some("Title-less table; caption alone is enough.".into());
    session.add_text_chunk(&table_b, "").expect("table 2");

    session
        .add_text_chunk(
            &TextHeader::paragraph(),
            "End. Uncaptioned floats are omitted from LOF/LOT.",
        )
        .expect("outro");
