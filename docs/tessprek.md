...
| Figure, biblio cite, quote, ref, slide, layout, toc, attachment, labeled box | `\figure{…}` / `\cite{…}` / `\quote{…}` / `\ref{…}` / `\slide{…}` / `\layout{…}` / `\toc{…}` / `\attach{…}` / `\box{…}` |
...
Labeled box. Attrs in braces; **one following paragraph** is the body (stops at
a blank line or the next command). Sealed as `TextRole::Callout` + `callout_kind`
+ optional `title`. Native PDF draws a left line today; a colored fill can use
...
\box{kind=abstract}
Full-width abstract. Keywords are `\box{kind=keywords title="Keywords"}`.
```
