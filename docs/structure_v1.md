...
anchors. Tessprek `\footnote{…}` / `\endnote{…}` seal as `InlineKind::Note`
(THI-396); display numbers are generated at print/HTML, so reordering never
leaves stale “Figure 3” / note-index text. Comments and review threads anchor
to a chunk id plus an optional span.
